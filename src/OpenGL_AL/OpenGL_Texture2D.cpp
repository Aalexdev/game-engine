...
namespace engine::OpenGL{
	...
	Ref<Texture2D> Texture2D::create(Renderer *renderer, uint32_t width, uint32_t height, TextureFormat format, void *data){
		return createRef<Texture2D>(renderer, width, height, format, data);
	}

	// Texture2D::Texture2D(const std::string &filepath, const Ref<Texture2DLibrary> &library) : library{library}{
	...
	Texture2D::Texture2D(Renderer *renderer, uint32_t width, uint32_t height, TextureFormat format, void *data) : renderer{renderer}, format{format}, width{width}, height{height}{
		ENGINE_PROFILE_FUNCTION();
	...
	void Texture2D::setData(void *data, uint32_t size, uint32_t offset){
		ENGINE_PROFILE_FUNCTION();
		glTextureSubImage2D(texture, 0, 0, 0, width, height, formatToGLDataFormat(format), GL_UNSIGNED_BYTE, data);
	}

	Renderer* Texture2D::getRenderer() const{
		return renderer;
	}
