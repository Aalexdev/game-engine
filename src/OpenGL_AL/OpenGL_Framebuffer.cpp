...
namespace engine::OpenGL{
	...
	Ref<Framebuffer> Framebuffer::create(Renderer *renderer, const FramebufferLayout &layout){
		return createRef<Framebuffer>(renderer, layout);
	}
	
	Framebuffer::Framebuffer(Renderer *renderer, const FramebufferLayout &layout){
		width = layout.width;
	...
	uint32_t Framebuffer::getColorAttachment(uint32_t slot) const{
		if (slot >= colorAttachments.size()) return 0;
		return colorAttachments[slot];
	}

	void Framebuffer::getPixel(uint32_t attachment, uint32_t x, uint32_t y, void* pixel){
	...
	void Framebuffer::clear(uint32_t attachment, void* clearData){
		ENGINE_CORE_ASSERT(attachment < colorAttachments.size(), "attchment index greater than the attachment count");
